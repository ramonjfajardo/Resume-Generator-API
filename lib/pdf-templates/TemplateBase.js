...
export const createResumeTemplate = (config) => {
    ...
    const styles = StyleSheet.create({
        ...
        headerLeft: {
            flex: 1,
            minWidth: 0,
            maxWidth: headerLayout === 'split' ? '48%' : '100%',
            paddingRight: 8,
        },
        name: {
        ...
        contact: {
            ...
            flexShrink: 0,
            width: headerLayout === 'split' ? '48%' : '100%',
            maxWidth: headerLayout === 'split' ? '48%' : '100%',
                    ...
                    <View style={styles.header}>
                        <View style={styles.headerContent}>
                            ...
                            <View style={styles.contact}>
                                {location && <Text wrap style={styles.contactItem}>{location}</Text>}
                                {phone && <Text wrap style={styles.contactItem}>{phone}</Text>}
                                {email && (
                                    <Link src={`mailto:${email}`} style={styles.contactItem}>
                                        <Text wrap>{email}</Text>
                                    </Link>
                                )}
                                {linkedin && (
                                    <Link src={linkedin} style={styles.contactItem}>
                                        <Text wrap>{linkedin}</Text>
                                    </Link>
                                )}
                                {github && (
                                    <Link src={github} style={styles.contactItem}>
                                        <Text wrap>{github}</Text>
                                    </Link>
                                )}
                            </View>
                        </View>
